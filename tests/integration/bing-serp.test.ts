...
describe("Bing SERP converter", () => {
  test("converts Bing search results", async () => {
    ...
    const result = await md.convert(path.join(FIXTURES, "test_serp.html"), {
      streamInfo: {
        charset: "utf-8",
    ...
    expect(result.markdown).toContain("1995–2007: Foray into the Web, Windows 95, Windows XP, and Xbox");
    ...
    expect(result.markdown).not.toContain("data:image/svg+xml,%3Csvg%20width%3D");
  });
});
